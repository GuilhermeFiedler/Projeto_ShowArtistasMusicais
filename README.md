# 🎵 Projeto Show de Artistas Musicais

Sistema desenvolvido em Java para cadastro e exibição de artistas musicais e seus shows, com foco na prática de Programação Orientada a Objetos (POO) e organização de dados.

---

## 🚀 Funcionalidades

- Cadastro de artistas musicais  
- Cadastro de shows  
- Associação de artistas aos shows  
- Listagem e exibição de informações dos artistas  

---

## 🛠️ Tecnologias Utilizadas

- Java (JDK 11)  
- Maven  
- Programação Orientada a Objetos (POO)  

---

## ▶️ Como Executar o Projeto

Execute a classe principal:

```java
src/main/java/Projeto_ShowArtistasMusicais/teste/Main.java
```

📌 Observações
- Certifique-se de ter o Java (JDK 11) instalado.
- Utilize o Maven para gerenciar as dependências.
- O projeto tem foco educacional, visando a prática de conceitos de POO.
